...
export async function getSortedPosts() {
  ...
  const sortedPosts = allPosts.sort((a, b) => {
    const aDate = a.data.published?.getTime() ?? 0
    const bDate = b.data.published?.getTime() ?? 0
    return aDate - bDate
  })
  return sortedPosts
}
...
abstract class PostsCollationGroup implements CollationGroup<'posts'> {
  ...
  sortCollationsMostRecent(): Collation<'posts'>[] {
    this.collations.sort((a, b) => {
      const aDate = a.entries[a.entries.length - 1].data.published?.getTime() ?? 0
      const bDate = b.entries[b.entries.length - 1].data.published?.getTime() ?? 0
      return bDate - aDate
    })
    return this.collations
  }
